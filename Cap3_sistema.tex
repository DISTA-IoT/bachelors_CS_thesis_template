...
Accanto ai diagrammi architetturali (che mostrano la struttura statica dei moduli), i \textbf{diagrammi di flusso (flowchart)} sono fondamentali per rappresentare visivamente la dinamica temporale, le diramazioni condizionali (\texttt{if-else}) e la sequenza delle operazioni. 

Un diagramma di flusso chiaro \textbf{sostituisce con enorme efficacia lunghe pagine di codice}, permettendo al lettore di cogliere la logica dell'algoritmo in pochi secondi.

La \Cref{fig:diagramma-flusso} illustra il flusso logico decisionale eseguito a ogni passo temporale.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{placeholder_flusso.png}
    \caption{Diagramma di flusso logico per ciascun timestep. Evidenzia la diramazione condizionale a fronte del superamento della soglia di anomalia e la convergenza verso la selezione dell'azione ottima.}
